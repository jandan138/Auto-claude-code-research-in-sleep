...
description: "Autonomously improve a generated paper via Claude-review MCP review → implement fixes → recompile, for 2 rounds. Use when user says \\\"改论文\\\", \\\"improve paper\\\", \\\"论文润色循环\\\", \\\"auto improve\\\", or wants to iteratively polish a generated paper."
---
...
If the context window fills up mid-loop, OpenCode auto-compacts. To recover, this skill writes `PAPER_IMPROVEMENT_STATE.json` after each round:
...
Send the full paper text to Claude-review MCP:
...
<details>
<summary>Claude-review MCP Review (Round 1)</summary>
...
<details>
<summary>Claude-review MCP Review (Round 2)</summary>
...
After each round's review AND at final completion, check `~/.opencode/feishu.json`:
...
- **Save FULL raw review text** — do not summarize or truncate Claude-review MCP responses
- **Use the `mcp__claude-review__review_reply_start` plus `mcp__claude-review__review_status`** for Round 2 to maintain conversation context
