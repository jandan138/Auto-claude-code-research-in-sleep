...
description: "Turn a vague research direction into a problem-anchored, elegant, frontier-aware, implementation-oriented method plan via iterative Claude-review MCP review. Use when the user says \"refine my approach\", \"帮我细化方案\", \"decompose this problem\", \"打磨idea\", \"refine research plan\", \"细化研究方案\", or wants a concrete research method that stays simple, focused, and top-venue ready instead of a vague or overbuilt idea."
---

> Override for OpenCode users who want **Claude Code CLI**, not a neutral review placeholder, to act as the reviewer. Install this package **after** `skills/skills-opencode/*`.
...
User input (PROBLEM + vague APPROACH)
  ...
  -> Phase 2 (Claude-review MCP): Review for fidelity, specificity, contribution quality, and frontier leverage
  -> Phase 3 (Claude): Anchor check + simplicity check -> revise method -> rewrite full proposal
  -> Phase 4 (Claude-review MCP, same thread): Re-evaluate revised proposal
...
- **REVIEWER_MODEL = `claude-review`** — Reviewer invoked through the local `claude-review` MCP bridge. Set `CLAUDE_REVIEW_MODEL` if you need a specific Claude model override.
...
| `threadId` | string or null | Reviewer thread ID for ``mcp__claude-review__review_reply_start`` continuity |
...
Send the full proposal to Claude-review MCP for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.
...
Send the revised proposal back to Claude-review MCP in the **same thread**:
...
[Full verbatim response via Claude-review MCP]
...
- **ALWAYS use `config: {"model_reasoning_effort": "xhigh"}`** for all Claude-review MCP calls.
- **Save `threadId` from Phase 2** and use `mcp__claude-review__review_reply_start` plus `mcp__claude-review__review_status` for later rounds.
...
```
skill tool → `research-refine-pipeline` → one-shot refine + experiment planning
skill tool → `idea-creator` with arguments: "direction"       -> candidate ideas
skill tool → `research-refine` with arguments: "PROBLEM: ... | APPROACH: ..."  <- you are here
skill tool → `experiment-plan` → detailed experiment roadmap
skill tool → `run-experiment` → execute the chosen method
skill tool → `auto-review-loop` → iterate on results and paper
```
