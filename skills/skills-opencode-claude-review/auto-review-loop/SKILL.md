...
description: "Autonomous multi-round research review loop. Repeatedly reviews via Claude review via `claude-review` MCP, implements fixes, and re-reviews until positive assessment or max rounds reached. Use when user says \"auto review loop\", \"review until it passes\", or wants autonomous iterative improvement."
---
...
After parsing the score, check if `~/.opencode/feishu.json` exists and mode is not `"off"`:
...
- Save threadId from the first review call and use the `mcp__claude-review__review_reply_start` plus `mcp__claude-review__review_status` for subsequent rounds
