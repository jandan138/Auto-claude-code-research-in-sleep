...
```
skill tool → `research-refine-pipeline` → one-shot method + experiment planning
skill tool → `research-refine` → method and claim refinement
skill tool → `experiment-plan` → detailed experiment roadmap
skill tool → `run-experiment` → execute the runs
skill tool → `auto-review-loop` → react to results and iterate on the paper
```
