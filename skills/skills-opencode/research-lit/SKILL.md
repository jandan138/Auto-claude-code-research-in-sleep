...
1. **Search by topic**: Use configured Zotero integration to find papers matching the research topic
...
[ -z "$SCRIPT" ] && SCRIPT=$(find ~/.opencode/skills/arxiv/ -name "arxiv_fetch.py" 2>/dev/null | head -1)
...
[ -z "$S2_SCRIPT" ] && S2_SCRIPT=$(find ~/.opencode/skills/semantic-scholar/ -name "semantic_scholar_fetch.py" 2>/dev/null | head -1)
