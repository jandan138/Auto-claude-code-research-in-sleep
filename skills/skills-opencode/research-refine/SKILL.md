...
- **REVIEWER_MODEL = `gpt-5.4`** — Reviewer model used via OpenCode review transport.
...
```
external reviewer transport (start):
...
- **Save `threadId` from Phase 2** and use the configured external reviewer continuation mechanism for later rounds.
