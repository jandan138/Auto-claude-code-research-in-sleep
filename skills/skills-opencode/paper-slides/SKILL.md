...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via OpenCode review transport for slide review.
...
### Phase 5: OpenCode review transport Review
...
> ⚠️ If no configured external reviewer transport is available, skip external review and proceed to Phase 6. Note the skip in `SLIDES_STATE.json`.
...
- **Feishu notifications are optional.** If `~/.opencode/feishu.json` exists, send notifications. If absent, skip.

## Parameter Pass-Through

```
skill tool → `paper-slides` with arguments: "paper/" — talk_type: oral, venue: ICML, minutes: 20, aspect: 4:3, notes: false
```
