...
```
skill tool → `monitor-experiment` with arguments: [server]
```
...
```
skill tool → `auto-review-loop` with arguments: "$ARGUMENTS — [chosen idea title]"
```

**What this does (up to 4 rounds):**
1. GPT-5.4 xhigh reviews the work (score, weaknesses, minimum fixes)
2. OpenCode implements fixes (code changes, new experiments, reframing)
