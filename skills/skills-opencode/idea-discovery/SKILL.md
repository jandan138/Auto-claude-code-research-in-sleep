...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via OpenCode review transport. Must be an OpenAI model (e.g., `gpt-5.4`, `o3`, `gpt-4o`). Passed to sub-skills.
...
```
skill tool → `idea-creator` with arguments: "$ARGUMENTS"
```
...
```
skill tool → `novelty-check` with arguments: "[top idea 1 description]"
skill tool → `novelty-check` with arguments: "[top idea 2 description]"
```
...
```
skill tool → `research-refine-pipeline` with arguments: "[top idea description + pilot results + reviewer feedback]"
```
...
- **Feishu notifications are optional.** If `~/.opencode/feishu.json` exists, send `checkpoint` at each phase transition and `pipeline_done` at final report. If absent/off, skip silently.
...
```
skill tool → `idea-discovery` with arguments: "direction"         ← you are here (Workflow 1, includes method refinement + experiment planning)
skill tool → `run-experiment` with arguments: ← deploy experiments from the plan
skill tool → `auto-review-loop` with arguments: "top idea"        ← Workflow 2: iterate until submission-ready
