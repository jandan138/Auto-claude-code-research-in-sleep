...
```
skill tool → `run-experiment` with arguments: "train model"       ← detects gpu: vast, calls `vast-gpu` provision
...
skill tool → `vast-gpu` with arguments: provision                 ← manual: analyze task + show options
skill tool → `vast-gpu` with arguments: rent <offer_id>           ← manual: rent a specific offer
skill tool → `vast-gpu` with arguments: list                      ← show active instances
skill tool → `vast-gpu` with arguments: destroy <instance_id>     ← tear down, stop billing
skill tool → `vast-gpu` with arguments: destroy-all               ← tear down everything
```
