...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via OpenCode review transport for plan review, figure review, writing review, and improvement loop.
...
```
skill tool → `paper-plan` with arguments: "$ARGUMENTS"
```
...
```
skill tool → `paper-figure` with arguments: "PAPER_PLAN.md"
```
...
```
skill tool → `paper-write` with arguments: "PAPER_PLAN.md"
```
...
```
skill tool → `paper-compile` with arguments: "paper/"
```
...
```
skill tool → `auto-paper-improvement-loop` with arguments: "paper/"
```

**What this does (2 rounds):**

**Round 1:** GPT-5.4 xhigh reviews the full paper → identifies CRITICAL/MAJOR/MINOR issues → OpenCode implements fixes → recompile → save `main_round1.pdf`

**Round 2:** GPT-5.4 xhigh re-reviews with conversation context → identifies remaining issues → OpenCode implements fixes → recompile → save `main_round2.pdf`
...
```
skill tool → `idea-discovery` with arguments: "direction"         ← Workflow 1: find ideas
implement                           ← write code
skill tool → `run-experiment` with arguments: ← deploy experiments
skill tool → `auto-review-loop` with arguments: "paper topic"     ← Workflow 2: iterate research
skill tool → `paper-writing` with arguments: "NARRATIVE_REPORT.md"  ← Workflow 3: you are here
                                         submit! 🎉
