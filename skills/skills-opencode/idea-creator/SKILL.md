...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via OpenCode review transport for brainstorming and review. Must be an OpenAI model (e.g., `gpt-5.4`, `o3`, `gpt-4o`).
...
Use the external LLM via OpenCode review transport for divergent thinking:
...
2. **Critical review**: Use REVIEWER_MODEL via the configured external reviewer continuation mechanism (same thread):
   ```
