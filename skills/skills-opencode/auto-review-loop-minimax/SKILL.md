...
description: "Autonomous multi-round research review loop using MiniMax API. Use when you want to use MiniMax instead of OpenCode review transport for external review. Trigger with \"auto review loop minimax\" or \"minimax review\"."
---
...
**API Key**: Read from `~/.opencode/settings.json` under `env.MINIMAX_API_KEY`, or from environment variable.

**Why MiniMax instead of OpenCode review transport?** Codex CLI uses OpenAI's Responses API (`/v1/responses`) which is not supported by third-party providers. See: https://github.com/openai/codex/discussions/7782
