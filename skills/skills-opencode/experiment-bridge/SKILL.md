...
- **OpenCode review transport unavailable** → skip silently, proceed to Phase 3 (graceful degradation)
...
```
skill tool → `run-experiment` with arguments: [sanity experiment command]
```
...
```
skill tool → `run-experiment` with arguments: [experiment commands]
```
...
```
skill tool → `idea-discovery` with arguments: "direction"          ← Workflow 1: find + refine + plan
skill tool → `experiment-bridge` with arguments: ← you are here (Workflow 1.5: implement + deploy)
skill tool → `auto-review-loop` with arguments: "topic"            ← Workflow 2: review + iterate
skill tool → `paper-writing` with arguments: "NARRATIVE_REPORT.md" ← Workflow 3: write the paper
