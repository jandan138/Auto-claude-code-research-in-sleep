...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via OpenCode review transport for poster review.
...
### Phase 6: OpenCode review transport Review
...
- **Feishu notifications are optional.** If `~/.opencode/feishu.json` exists, send notifications. Otherwise skip.
...
```
skill tool → `paper-poster` with arguments: "paper/" — venue: CVPR, size: A1, orientation: portrait, columns: 3
```
