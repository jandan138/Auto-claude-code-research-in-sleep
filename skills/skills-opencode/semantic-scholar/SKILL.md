...
[ -z "$SCRIPT" ] && SCRIPT=$(find ~/.opencode/skills/semantic-scholar/ -name "semantic_scholar_fetch.py" 2>/dev/null | head -1)
```
...
```text
skill tool → `arxiv` with arguments: "topic"           - search arXiv preprints (complements this search)
skill tool → `research-lit` with arguments: "topic"    - multi-source review: Zotero + local PDFs + arXiv + S2
skill tool → `novelty-check` with arguments: "idea"    - verify novelty against literature
```
