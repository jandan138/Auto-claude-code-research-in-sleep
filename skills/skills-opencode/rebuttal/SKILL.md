...
- **REVIEWER_MODEL = `gpt-5.4`** — Used via OpenCode review transport for internal stress-testing.
- **MAX_INTERNAL_DRAFT_ROUNDS = 2** — draft → lint → revise.
- **MAX_STRESS_TEST_ROUNDS = 1** — One OpenCode review transport critique round.
...
2. Invoke ``experiment-bridge`` with the mini plan:
   ```
   skill tool → `experiment-bridge` with arguments: "rebuttal/REBUTTAL_EXPERIMENT_PLAN.md"
   ```
...
### Phase 6: OpenCode review transport Stress Test
...
5. Use OpenCode review transport reply for continuity if useful
