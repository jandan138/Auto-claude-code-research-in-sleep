...
skill tool → `idea-discovery` with arguments: ────┤
...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via OpenCode review transport for proposal review. Must be an OpenAI model (e.g., `gpt-5.4`, `o3`, `gpt-4o`).
...
```
skill tool → `grant-proposal` with arguments: "topic — GENERIC, sections: Background|Methods|Impact, language: English"
```
...
```
skill tool → `research-lit` with arguments: "$ARGUMENTS"
```
...
- Run ``novelty-check`` on the proposed research direction to verify the gap is real:
  ```
  skill tool → `novelty-check` with arguments: "[proposed gap statement]"
  ```
...
```
skill tool → `research-review` with arguments: "[GRANT_TYPE] [GRANT_SUBTYPE] proposal structure:
Gap: [gap statement]
...
```
skill tool → `paper-illustration` with arguments: "Overview diagram showing [aims relationship + shared resources] for grant proposal"
```
...
```
skill tool → `research-review` with arguments: "Complete [GRANT_TYPE] [GRANT_SUBTYPE] proposal draft. Evaluate as a [GRANT_TYPE] review panelist using official criteria. [PASTE FULL PROPOSAL TEXT]"
```
...
> ⚠️ **OpenCode review transport fallback**: If no configured external reviewer transport is available, skip external review. Note "External review skipped — no OpenCode review transport available. Consider running ``auto-review-loop-llm`` separately." in GRANT_REVIEW.md. The proposal is still usable without external review.

If ``research-review`` is invoked (preferred), it handles the external reviewer call internally. If using the configured external reviewer continuation mechanism directly (e.g., to maintain thread context from Phase 2):
...
Implement CRITICAL and MAJOR fixes. If MAX_REVIEW_ROUNDS > 1, re-submit for another round via the configured external reviewer continuation mechanism.
...
- **Feishu notifications are optional.** If `~/.opencode/feishu.json` exists, send `checkpoint` at each phase transition and `pipeline_done` at final output. If absent, skip silently.
...
```
skill tool → `grant-proposal` with arguments: "topic — KAKENHI Start-up, sources: zotero, arxiv download: true"
```
...
| `reviewer model` | gpt-5.4 | Codex review model | → OpenCode review transport |
...
```
skill tool → `idea-discovery` with arguments: "direction"              ← Workflow 1: find validated ideas
skill tool → `research-refine` with arguments: "idea"                  ← sharpen the method
skill tool → `grant-proposal` with arguments: "idea — KAKENHI"         ← this skill: write the grant proposal
                                          ← [submit & get funded]
skill tool → `experiment-bridge` with arguments: ← implement experiments with funding
skill tool → `auto-review-loop` with arguments: "results"              ← Workflow 2: iterate until submission-ready
skill tool → `paper-writing` with arguments: ← Workflow 3: write the paper
```
