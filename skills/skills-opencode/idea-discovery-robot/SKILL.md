...
- **REVIEWER_MODEL = `gpt-5.4`** — External reviewer model via OpenCode review transport
...
```
skill tool → `research-lit` with arguments: "$ARGUMENTS — focus venues: CoRL, RSS, ICRA, IROS, RA-L, TRO, Science Robotics"
```
...
```
skill tool → `idea-creator` with arguments: "$ARGUMENTS — robotics frame: [paste Robotics Problem Frame] — focus venues: CoRL, RSS, ICRA, IROS, RA-L — benchmark-specific ideas only — sim-first pilots — no real-robot execution without explicit approval — require failure metrics and baseline clarity"
```
...
```
skill tool → `novelty-check` with arguments: "[idea description with embodiment + task family + benchmark + sensor stack + controller/policy class + sim2real angle + target venues: CoRL/RSS/ICRA/IROS/RA-L]"
```
...
```
skill tool → `research-review` with arguments: "[top idea with robotics framing, embodiment, benchmark, baselines, pilot plan, evaluation metrics, and sim2real/hardware risks — review as CoRL/RSS/ICRA reviewer]"
```
...
```
skill tool → `idea-discovery-robot` with arguments: "direction"   ← you are here
implement sim-first pilot
skill tool → `run-experiment` with arguments: ← if infrastructure exists
skill tool → `auto-review-loop` with arguments: "top robotics idea"
```
