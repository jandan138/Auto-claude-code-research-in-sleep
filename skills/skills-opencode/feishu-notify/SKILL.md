...
The skill reads `~/.opencode/feishu.json`. If this file does not exist, **all Feishu functionality is disabled** — skills behave exactly as before.
...
```bash
cat ~/.opencode/feishu.json 2>/dev/null
```
...
Check if `~/.opencode/feishu.json` exists and mode is not "off":
